...
class ModelHandler:
    ...
    def createArchitecture(self,model_type,num_classes,input_shape,chain,strategy,fusion):
        ...
        if(model_type == 'coord_mlp'):
            ...
            out = layer = Conv2D(2*channel, (3, 3), padding="SAME", activation='relu', name='img_conv9')(layer)

            layer = Flatten(name='img_flatten')(layer)
            layer = Dense(4*num_classes, activation='relu', name='img_dense1')(layer)
            layer = Dropout(0.5, name='img_dropout4')(layer)
            layer = Dense(num_classes, activation='relu', name='img_dense2')(layer)
            layer = Dropout(0.5, name='img_dropout5')(layer)
